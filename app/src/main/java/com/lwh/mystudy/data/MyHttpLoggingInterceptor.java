...
import com.lwh.mystudy.utils.LogUtils;
...
public final class MyHttpLoggingInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        final HttpLogCache.HttpLogger httpLogger = HttpLogCache.getHttpLogger(request.url()
                .toString());
        ...
        if (logHeaders) {
            ...
            if (!logBody || !hasRequestBody) {
                ...
                for (MultipartBody.Part part : multipartBody.parts()) {
                    RequestBody partResponseBody = part.body();
                    if (partResponseBody.contentType() != null) {
                        logger.log("Content-Type: " + partResponseBody.contentType());
                    }
                    if (partResponseBody.contentLength() != -1) {
                        logger.log("Content-Length: " + partResponseBody.contentLength());
                    }
                    Headers partHeaders = part.headers();
                    for (int i = 0, count = partHeaders.size(); i < count; i++) {
                        String name = partHeaders.name(i);
                        // Skip headers from the request body as they are explicitly logged above.
                        if (!"Content-Type".equalsIgnoreCase(name) && !"Content-Length"
                                .equalsIgnoreCase
                                        (name)) {
                            logger.log(name + ": " + partHeaders.value(i));
                        }
                    }
                    if (bodyEncoded(partHeaders)) {
                        logger.log(" (encoded body omitted)");
                    } else {
                        Buffer buffer = new Buffer();
                        partResponseBody.writeTo(buffer);

                        Charset charset = UTF8;
                        MediaType contentType = partResponseBody.contentType();
                        if (contentType != null) {
                            charset = contentType.charset(UTF8);
                        }
                        if (isPlaintext(buffer)) {
                            if (buffer.size() > 1000) {
                                logger.log("(part body is too long)");
                            } else {
